...
# 業務分野別のスキル。
#
# Daigasはタレントパレット上で全業務を18領域に分類し、過去45年分・
# 約12,000チーム分の組織名を紐づけて約5,000名の経験領域を可視化済み
# （ベンダー公表事例）。公開されている領域名は「ガス製造」「人事総務」の2つのみ。
#
# したがって以下は事業内容から推定した仮置きであり、社内の18領域分類とは
# 別物である。実運用では**タレントパレットの18領域に差し替える**こと。
# 既に整備済みのデータがあるので、自前でスキル体系を作る必要はない。
#   出典: https://www.pa-consul.co.jp/talentpalette/case/details/osakagas.html
SKILL_POOL = {
    "ガス製造・エンジニアリング": [
