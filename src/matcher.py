...
def hard_filter(emp: Employee, pos: Position) -> tuple[bool, str]:
    ...
    # 転居の可否。事業所が地域に分散する事業では、勤務地が変わる異動が
    # 日常的に発生する。転居できない社員をスコアで扱うと
    # 「他のスコアが高ければ転居不可の人が上位に出る」ことになり、
    # 提示された時点で本人にも受け入れ先にも無駄が生じる。
    home = emp.home_location or emp.location
    if not emp.relocatable and (pos.requires_relocation or pos.location != home):
        return False, f"転居不可(生活拠点{home}/勤務地{pos.location})"
...
def score_pair(emp: Employee, pos: Position, graph: dict,
               net: "NetworkContext | None" = None) -> "MatchResult | None":
    ...
    r = MatchResult(
        ...
        network_score=nw, known_peers=peers,
...
def build_reasons(r: "MatchResult") -> list[str]:
    ...
    if r.known_peers:
        reasons.append(
            f"異動先に過去の同僚が{r.known_peers}名（立ち上がりの支えになりうる）")
    if r.missing_skills:
    ...
    # 育成コースの不一致は「除外」ではなく「注意喚起」にとどめる。
    # コースは本人が選び直せるものなので、ハードフィルタで落とすと
    # コース変更を伴うキャリアチェンジが構造的に不可能になる。
    course = r.position.career_course
    if course and r.employee.career_course and course != r.employee.career_course:
        reasons.append(
            f"※育成コースが異なる（本人{r.employee.career_course}"
            f"／ポジション{course}）。コース変更の意向確認が必要")
    return reasons
...
def match_candidates_for_position(
    ...
    for emp in employees:
        ...
        r = score_pair(emp, pos, graph, net)
        if r is None:
